...
No credentials needed to see the output: `python demo/generar_dashboard_demo.py`
generates `demo/dashboard_demo.html` from 60 synthetic transactions (fictional
merchants, no real spending data) using the same `generate_dashboard()`
function as the production pipeline.

![Dashboard with synthetic data](docs/img/dashboard.png)
*Category breakdown, daily spend, and a searchable transaction table - all
from fictional data generated by `demo/generar_dashboard_demo.py`.*

### Firefly III itself

The screenshots above are of `finanzas_bpd.py`'s own dashboard. What actually
holds the budget long-term is [Firefly III](https://www.firefly-iii.org/), the
self-hosted tool this pipeline pushes categorized transactions into (see
Architecture above). These are captured from a disposable local instance
seeded with the same kind of fictional data - not the author's real budget.

![Firefly III dashboard](docs/img/firefly_dashboard.png)
*Net worth trend and account balances over a month, driven entirely by
synthetic transactions pushed through the Firefly III API.*

![Firefly III budgets](docs/img/firefly_presupuestos.png)
*Spend-per-budget tracking - one of the views `sync_all.py` keeps up to date
without any manual entry.*

![Firefly III transactions](docs/img/firefly_transacciones.png)
*The transaction list Firefly III builds from what `finanzas_bpd.py` pushes:
category and budget already assigned, searchable and filterable.*
